  ...
  return (
    <section className="relative w-full h-[100svh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 container-max flex flex-col items-start justify-end h-full pb-20 sm:pb-24 md:justify-center md:pb-0 max-w-3xl">
        ...
        >
          BREAK<br className="sm:hidden" /> THE<br className="sm:hidden" /> RULES
        </motion.h1>
      ...
      {/* Scroll Indicator — desktop only, hide on scroll */}
      <motion.div
        className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 hidden md:flex flex-col items-center gap-2"
        animate={{ y: isScrolled ? [0, 0] : [0, 8, 0], opacity: isScrolled ? 0 : 1 }}
        transition={{ duration: isScrolled ? 0 : 2, repeat: isScrolled ? 0 : Infinity, ease: "easeInOut" }}
      >
